...
const ModernStepGuide: React.FC = () => {
    ...
    (entries: IntersectionObserverEntry[]) => {
      entries.forEach((entry) => {
        ...
        if (entry.isIntersecting && index !== -1) {
          ...
          console.log(`Active step: ${index}`) // Debug log
        }
      })
  ...
  useEffect(() => {
    const observer = new IntersectionObserver(handleIntersection, {
      ...
      rootMargin: '-20% 0px', // Trigger earlier on mobile
    })

    stepRefs.current.forEach((ref, index) => {
      if (ref) {
        observer.observe(ref)
        console.log(`Observing step ${index}`) // Debug log
      }
    })
    ...
    <div className='relative bg-black text-white min-h-screen'>
      <div className='lg:hidden block touch-action-auto'>
        <div ref={containerRef} className='relative px-4 py-8 min-h-screen'>
          {/* Timeline */}
          <div className='absolute left-6 top-0 h-full w-[2px] bg-neutral-700 z-20'>
            <div
              className='bg-gradient-to-b from-violet-500 via-cyan-500 to-purple-500 w-full transition-all duration-1000 ease-out z-30'
            ...
            {steps.map((step, index) => (
              ...
              >
                ...
                <div className='ml-12'>
                  ...
                  <h2
                    className='text-2xl font-bold mb-3 transition-all duration-500 ease-in-out 
                         text-white !opacity-100'
                  >
                    {step.title}
                  </h2>
                  <p className='text-base leading-relaxed transition-all duration-500 ease-in-out '>
                    {step.description}
                  </p>
